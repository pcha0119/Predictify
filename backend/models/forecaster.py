...
_SHORT_FOLD_NOTICE_EMITTED = False
...
def _walk_forward_folds(
    ...
    global _SHORT_FOLD_NOTICE_EMITTED
    if not folds:
        # For short series, create a best-effort single split even when
        # min_train_rows cannot be satisfied. This avoids empty-fold CV runs.
        split_fallback = max(int(n * 0.7), 2)
        split_fallback = min(split_fallback, n - 1)
        if n >= 4 and split_fallback >= 2:
            folds.append((df.iloc[:split_fallback].copy(), df.iloc[split_fallback:].copy()))

        if not _SHORT_FOLD_NOTICE_EMITTED:
            _SHORT_FOLD_NOTICE_EMITTED = True
            logger.warning(
                "walk_forward_folds: not enough data for full walk-forward "
                "(n=%d, horizon=%d, min_train=%d). Using a single fallback split.",
                n, horizon, min_train_rows,
            )
        else:
            logger.debug(
                "walk_forward_folds short-series fallback (n=%d, horizon=%d, min_train=%d).",
                n, horizon, min_train_rows,
            )
